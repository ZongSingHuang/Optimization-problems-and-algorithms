...
def fitness(X):
    ...
    P = X.shape[0]
    X = X.reshape(P, -1, 2).astype(int) # int
    
    decoded_X = np.packbits(X, bitorder='little', axis=2).reshape(P, -1) # uint8
    decoded_X = ( 10 * ( 1 + decoded_X ) ).astype(float) # float
    
    F = np.sum(decoded_X**2, axis=1)
